				...
				<section class="mainContent">
					<div class="productRow">
						...
						try {
							...
							String EarphoneID = request.getParameter("EarphoneID");
							//out.print(aid);
							//
							String str = "SELECT * FROM Earphone WHERE EarphoneID = " + EarphoneID;
							...
							str = "select InitialPrice,max(price)as price,AuctionID,Name,Brand,Pic FROM Earphone join Auction using (EarphoneID) left join `Account_Bid_Auction` using(auctionID) WHERE Brand = '" + Brand + "' || type = '" + Type + "' group by AuctionID order by startdate desc";
							//out.print(str);
							...
							while (result.next()) {
								%><article class="productInfo">
									<a href="Auctions.jsp?aid=<%=result.getString("AuctionID") %>">	
										<div><img alt="sample" src="<%=result.getString("Pic")%>"></div>
										<p class="price">$<%=result.getString("Price") %></p>
										<p class="productContent"><%=result.getString("Name") %>&nbsp;(<%=result.getString("Brand")%>) </p>
									</a>
								</article>
